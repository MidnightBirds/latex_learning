\documentclass{ctexart}

\usepackage{array, tabularx}

\begin{document}
%  \\只用于分行(可选择增加竖直间距)
% \newline ：强制换行
% \par ：段落结束，开启新段落

% \newpage：强制分页
% \clearpage：强制分页并清除浮动对象

% 断词符号 \-

% 断行或断页的适合度 \linebreak[n] \pagebreak[n] \nolinebreak[n] \nopagebreak[n]

% \input{文件名}：将指定文件的内容插入到当前位置

% 空格的处理 
%英文之间直接输入空格
%中文和英文之间会自动处理空格
%中文之间的空格会被忽略
%不会断行的空格：~
%特殊字符
% \# \$ \& \% \_ \{ \} \~{} \^{}  \textbackslash 

% 英文连字  {} 用于阻止连字

%引号  `'  ``''

%横线 - 连字符 -- 短破折号 --- 长破折号

%省略号 \ldots \dots

%• article 文档类带编号的层级为 \section、\subsection、\subsubsection 三级；
%• report 和 book 文档类带编号的层级为 \chapter、\section、\subsection 三级

% 生成目录： \tableofcontents

% 标题页： \title{title} \author{author} \date{date} \maketitle

% 交叉引用： \label{label} \ref{label} \pageref{label}
% \label{} 位置： 

%脚注 \footnote{脚注内容}
%边注 \marginpar{\footnotesize 边注内容}   字数更小

%列表 有序列表 列表可以嵌套使用，最多嵌套四层
\begin{enumerate}
\item[⟨item title⟩] …
\end{enumerate}
%有序列表的符号由命令 \labelenumi 到 \labelenumiv 定义
% \renewcommand{\labelenumi}%
% {\Alph{enumi}>}


%无序列表
\begin{itemize}
\item[⟨item title⟩] …
\end{itemize}
%符号由命令 \labelitemi 到 \labelitemiv 定义
%\renewcommand{\labelitemi}{\ddag}
%\renewcommand{\labelitemii}{\ddag}

%描述列表
\begin{description}
\item[⟨item title⟩] …
\end{description}

%对齐环境
\begin{flushleft}
\end{flushleft}

\begin{flushright}
\end{flushright}

\begin{center}
\end{center}
%改变对齐方式，使用后要调整回来
%\centering
%\raggedleft
%\raggedright

%引用环境
\begin{quote}
\end{quote}

\begin{quotation}
\end{quotation}

\begin{verse}
\end{verse}

%摘要环境
\begin{abstract}
\end{abstract}


%代码环境
% 回车和空格也分别起到换行和空位的作用
\begin{verbatim}
#include <iostream>
int main()
{
    std::cout << "Hello, world!"
    << std::endl;
    return 0;
}
\end{verbatim}
%\verb 命令   排版简短的代码或关键字
\verb⟨delim⟩⟨code⟩⟨delim⟩
%⟨delim⟩习惯上使用 | 符号

%表格 tabular
%列格式：
% |c| 居中
% |l| 左对齐
% |r| 右对齐
% p{⟨width⟩} 自定义宽度
\begin{tabular}{⟨列格式⟩ p{⟨width⟩}}
  ⟨表格内容⟩
\end{tabular}
%高级使用表格 array宏包 tabularx宏包
%X 列格式 自动计算列宽
\begin{tabularx}{14em}
  {|*{4}{>{\centering\arraybackslash}X|}}
  \hline
  A & B & C & D \\
  \hline
  ABC & BCD & CDE & DEF \\
  \hline
\end{tabularx}
%\cline{⟨i⟩-⟨j⟩} 用来绘制跨越部分单元格的横线
%合并单元格
%\multicolumn{⟨n⟩}{⟨column-spec⟩}{⟨item⟩}

%图片 
% 需要调用graphicx宏包
%\includegraphics[⟨options⟩]{⟨filename⟩}
%option选项
% width=⟨length⟩ 设置图片宽度
% height=⟨length⟩ 设置图片高度
% scale=⟨factor⟩ 缩放图片
% angle=⟨angle⟩ 旋转图片

% 盒子
%水平盒子
%\makebox[⟨width⟩][⟨align⟩]{…}

%带框的盒子
%\framebox[⟨width⟩][⟨align⟩]{…}

%可以换行的盒子
%\parbox[⟨align⟩][⟨height⟩][⟨inner-align⟩]{⟨width⟩}{…}
\begin{minipage}[⟨align⟩][⟨height⟩][⟨inner-align⟩]{⟨width⟩}
…
\end{minipage}

%实心盒子
%\rule[⟨raise⟩]{⟨width⟩}{⟨height⟩}

% 浮动体   大块的内容可以脱离上下文设置
% h 当前位置（代码所处的上下文）
% t 顶部
% b 底部
% p 单独成页
% ! 在决定位置时忽视限制
\begin{table}[⟨placement⟩]
  
\end{table}
%\caption{…} 添加标题 （内部使用）



%排版数学公式
% 数学模式 数学模式中输入的空格被忽略 \quad为空格
\usepackage{amsmath}
%行内公式 与文字混排
% 用一对$$包围
The Pythagorean theorem is
$a^2 + b^2 = c^2$.
%行间公式 分行，生成编号
\begin{equation}
  % \tag 命令手动修改公式的编号
  % \notag 取消为公式编号
  a^2 + b^2 = c^2
\end{equation}

%行间公式 将公式用命令 \[ 和 \] 包裹  不带编号
\[ a^2 + b^2 = c^2 \]

%如果想正常打文本
%\textbf{} 加粗
%\text{} 文本

%指数 ^{}  下标 _{}
%导数符号'(′) 是一类特殊的上标，可连续使用，但只能在其后添加其他上标

%分式 \frac{}{}  根式 \sqrt[]{}   二项式 \binom{}{}

%关系符号  =，>，<直接输入
%特殊  \ne \ge \le \approx \equiv \propto \sim

%算符  +、−、∗、/直接输入
%特殊 乘号 × (\times)、除号 ÷ (\div)、点乘 · (\cdot)、加减号 ± (\pm) / ∓(\mp)

%巨算符 \int 积分号 \sum 求和号 \oint 圈积分号 \prod 乘积号
%巨算符的上下标位置可由 \limits（上下方） 和 \nolimits（右上方右下方） 调整 

%重音 一般应当对某个符号而不是“符号加下标”使用重音
% \dot{}  \ddot{}  \vec{}

% \overbrace 和 \underbrace 命令用来生成上/下括号，各自可带一个上/下标公式

% 长公式折行  multline环境允许折行   多行公式的首行左对齐，末行右对齐，其余行居中

% 数组和矩阵
%二维数组 array环境
%通常外套括号 定界符
\begin{array}{cols}
  
\end{array}

%amsmath中的矩阵
%matrix环境   定界符的矩阵 pmatrix  bmatrix Bmatrix vmatrix Vmatrix

%数字字母字体 \mathcal{}花体   \mathfrak{}德文尖角体    \mathbb{}黑板粗体

% 粗斜体  \boldsymbol{}

% 数学符号的尺寸  从大到小
% \displaystyle 显示模式
% \textstyle 默认模式
% \scriptstyle 小型模式
% \scriptscriptstyle 小型小型模式

%定理环境
%\theoremstyle 命令支持定理格式的切换 在\newtheorem 前使用
%建立环境
% \newtheorem{envname}{title}[section-level]

%使用
\begin{envname}
\end{envname}

%证明环境和证毕符号
\begin{proof}
\end{proof}


%排版样式设定
%字体大小 \tiny \scriptsize \footnotesize \small \normalsize \large \Large \LARGE \huge \Huge
%常用{}控制范围
%\fontsize{size}{skip}
%\fontsize 的设定需要 \selectfont 命令才能立即生效

%字体样式 \textrm{} \textit{} \textsc{} \texttt{} \textsf{} \textbf{}


%文字修饰与强调
%下划线 \underline{}    不能分行
%ulem宏包中 \uline{}

%强调 \emph{}  文字变为斜体以示强调

% 长度与长度变量
% 弹性长度 例如： 12pt plus 2pt minus 1pt

% 长度单位
% pt  磅 1/72.27
% in  英寸
% cm  厘米
% mm  毫米
% bp  1/72
% ex  当前字体的x高度
% em  当前字体的M宽度
% sp  1/65536磅
% 缩进 \indent \noindent

%自定义长度变量
\newlength{\mylen}
\setlength{\mylen}{lenght}
\addtolength{\mylen}{lenght}

% 行距

\end{document}